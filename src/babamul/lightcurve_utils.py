...
band_colors = {'g': 'green', 'r': 'red', 'i': 'orange', 'z': 'purple', 'y': 'brown', 'u': 'blue'}
...
def plot_lightcurve(alert: dict[str, Any], ax: plt.Axes | None = None, show: bool = True):
    """
    ...
    for band in df['band'].unique():
        ...
        # Plot detections
        if len(detections) > 0:
            ax.errorbar(detections['mjd'], detections['mag'],
                        yerr=detections['magerr'],
                        fmt='o', color=color, label=f'{band}-band',
                        markersize=8, capsize=3)

        # Plot upper limits
        if len(limits) > 0:
            ax.scatter(limits['mjd'], limits['mag'],
                        marker='v', color=color, alpha=0.3, s=50)
    ...
    if show:
        ax.set_title(f"Lightcurve for {alert['objectId']}", fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.show()
